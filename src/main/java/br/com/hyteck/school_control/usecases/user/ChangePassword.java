...
public class ChangePassword {
    ...
    public void execute(String username, PasswordChangeRequest request){
        ...
        // 1. Verificar Senha Atual (Temporária)
        if (!passwordEncoder.matches(request.currentPassword(), user.getPassword())) {
            logger.warn("Senha atual incorreta fornecida para o usuário: {}", username);
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Senha atual incorreta.");
            // Considere adicionar limites de tentativa aqui para evitar brute-force
        }

        // 2. Validar Nova Senha (opcional, mas recomendado - ex: não ser igual à antiga, complexidade)
